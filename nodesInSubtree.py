...
def visit(root, query, s, pointers):
    pointers[root.val] = root
    count = 0
    if query[1] == s[root.val - 1]:
        count = 1
    for child in root.children:
        count += visit(child, query, s, pointers)
    return count

# Add any helper functions you may need here
def search(root, query, s, pointers):
    pointers[root.val] = root
    if query[0] == root.val:
        return visit(root, query, s, pointers)
    for child in root.children:
        count = search(child, query, s, pointers)
        if count != 0:
            return count
...
def count_of_nodes(root, queries, s):
    ...
    for qn, query in enumerate(queries):
        p = pointers[query[0]]
        if p is None:
            results[qn] = search(root, query, s, pointers)
        else:
            results[qn] = visit(p, query, s, pointers)
